struct SuffixAutomaton {
    ...
    long long substrcntdp(int ind) // number of distinct substrings
    {
        auto &ret = substrcnt[ind];
        if(ret!=-1)return ret;
        if(ind!=0)ret = 1;
        else ret = 0;
        for(auto i: t[ind].nxt){
            ret += substrcntdp(i.second);
        }
        return ret;
    }
};
